...
public class Graph {
	...
	public static String [] propertyTagPairs = {
		...
		XMLBackgroundImageStartTag,XMLBackgroundImageEndTag,
		XMLFileNameStartTag,XMLFileNameEndTag,
		XMLDimensionsStartTag,XMLDimensionsEndTag
	};
	...
	//public static Color NORMAL_EDGE_COLOR = Color.black; //black
	public static Color SELECTED_COLOR = Color.red; 
	public static Color MARKED_COLOR = new Color(100,149,237); //cornflower blue	
	...
	//(some graphs are models based on a background images, like
	//a map)
	...
	public static enum PathFindingMethod {
		...
		AStarLowerBound,
		AStarManhattan,
		AStarZeroCostHeuristic,
		AStarBestFirstGreedy,
		GreedyBestNeighbour
	}

	public static String nameOfMethod(PathFindingMethod aPathFindingMethod){
		if(aPathFindingMethod == PathFindingMethod.Dijkstra) return "Dijkstra";
		else if(aPathFindingMethod == PathFindingMethod.AStarLowerBound) return "AStarLowerBound";
		else if(aPathFindingMethod == PathFindingMethod.AStarManhattan) return "AStarManhattan";
		else if(aPathFindingMethod == PathFindingMethod.AStarZeroCostHeuristic) return "AStarZeroCostHeuristic";
		else if(aPathFindingMethod == PathFindingMethod.AStarBestFirstGreedy) return "AStarBestFirstGreedy";
		else if(aPathFindingMethod == PathFindingMethod.GreedyBestNeighbour) return "GreedyBestNeighbour";
		return "UNKNOWN";
	...
	// The get & set methods=============================================================
	public static double infinity() {return Double.POSITIVE_INFINITY;}
	...
	public ArrayList<Node> copyOfNodes(){ 
		//return a copy of the nodes List for use with
	...
	public ArrayList<Edge> copyOfEdges(){
		//return a copy of the edge list for use with
		//destructive algorithms.
	...
	public Edge edgeAt(Point p) {
		...
		/*This code basically gets the distance between the start node of the edge and 
    the point on the screen selected as well as the distance between the end 
    node of the edge and that point. After that, I add these two values together 
    and I subtract the distance between the 2 nodes. If the user clicks 
    sufficiently close to the edge (in my code I made it a distance smaller than 
    1) then that edge is then selected. 
    ...
    Thanks to: Adrian Batos-Parac for this method
		...
		for (int i=0; i<edges.size(); i++) {
			...
			double distanceToStart = Graph.distanceBetween(p, 
					anEdge.getStartNode().getLocation());
			double distanceToEnd = Graph.distanceBetween(p, 
					anEdge.getEndNode().getLocation());
			double totalChange = (distanceToStart + distanceToEnd) - 
					(Graph.distanceBetween(anEdge.getEndNode().getLocation(), 
							anEdge.getStartNode().getLocation()));
	...
	public ArrayList<Edge> edgesBetween(ArrayList<Node> nodes1, ArrayList<Node> nodes2) {
		...
		for(Edge anEdge : edges) {
			...
			if ((nodes1.contains(startNode)) &&(nodes2.contains(endNode))) partitionEdges.add(anEdge);
			else if ((nodes1.contains(endNode)) &&(nodes2.contains(startNode))) partitionEdges.add(anEdge);
		}
		...
		for(Node aNode1 : nodes) {
			for(Node aNode2 : nodes){
				if(edgeBetween(aNode1,aNode2) == null){
					if(aNode1 != aNode2) addEdge(aNode1,aNode2);
				}
			}
	...
	public void findMinimumCostSpanningTree() {
		...
		if(remainingNodes.size() <= 1) return; //no tree possible

		//partition the nodes into a partition of 1 node and the remaining nodes
	...
	public void findMinimumCostSpanningTree(ArrayList<Node> processedNodes, ArrayList<Node> remainingNodes) {
		...
		//Find a minimum cost spanning tree recursively by finding the lowest cost edge
		//that connects the partial tree represened by the processed nodes to one
		...
		if(crossEdges.size()<1) return; //graph is disconnected

		Edge minimumCostEdge = Edge.removeMinimumWeightEdge(crossEdges); //good place for priority queue
	...
	private boolean hasPathFrom(Node source, Node target) {
		//         ===========
		//WARNING: don't call this method directly, call via the hasSTPath() method
		//Answer true if there is a path from the source node to the target
		//node. Answer false if no path exists (source and target are disconnected)

		//Basis case
		if (source == target) return true; //we have reached the target
		//Recursive case
		...
		for(Node neighbour : source.neighbours()) {
			if(neighbour.getVisited() == false){
				if(hasPathFrom(neighbour, target)) return true;
			}
	...
	public void depthFirstSearch(Node source, int level_counter) {
		...
		for(Node neighbour : source.neighbours()) {
			if(neighbour.getVisited() == false){
				...
				edgeBetween(source, neighbour).setSelected(true); //select the forward edges

			}
			else
				edgeBetween(source, neighbour).setMarked(true); //mark the back edges
	...
	public void depthFirstSearch() {
		...
		Node source = null;
		if (selectedNodes.size() >= 1) source = selectedNodes.get(0);
		else source = nodes.get(0);
	...
	public void breadthFirstSearch(ArrayList<Node> nodesToSearch, int level_counter) {
		...
		//Basis case
		if(nodesToSearch.size() < 1) return;
	...
	public void maxFlow(){
		//This method determines the maximum flow between the source node and target node.
		//The edge weights are used as the maximum capacity that an edge can carry.
		//The flow and direction variables in the edges are used to label the amount of flow each
		//edge is carrying.

		//This method also displays the amount that on of the bottleneck edge's capacity should be increased
	...
	public void draw(Graphics2D aPen) {
		...
		for (int i=0; i<nodes.size(); i++){
			...
			if(GraphEditor.getDisplayNodeLabels()) nodes.get(i).drawNodeLabel(aPen); 
		} 
		...
		for(Edge e: this.getEdges()){
			if (e.isSelected()) weight += e.getWeight();
		}
		...
		for(Node n : this.getNodes()){
			if(n.isSelected()) weight += n.getDistance();
		}
	...
	public boolean parsePropertyString(String aString){
		...
		for(int i=0; i<propertyTagPairs.length/2; i++){
			...
			if((aString.startsWith(beginTag)) && (aString.endsWith(endTag))){
				...
				setProperty(beginTag, aString.substring((beginTag.length()),(aString.length() - endTag.length()-1)).trim());
				return true;
			}
	...
	public void writeToFile(String baseIndent, PrintWriter outputFile){
		...
		if(backgroundImageFileName != null && !backgroundImageFileName.isEmpty()){
			...
			propertyString = XMLDimensionsStartTag + backgroundImageWidth + "," + backgroundImageHeight + XMLDimensionsEndTag;            
			outputFile.println(indent  + tab + propertyString);
			...
			{ 
				...
				if(dataString.length() ==0){
					...
					if(edgeModel != null) {
						...
						//Replace the parsed temporary nodes with nodes from the graph at the
						//same location.
						//This is because the nodes of the graph were not known when the edge was
						//parsed in from the xml file, only the locations were known
	...
	public ArrayList<Edge> findPath(PathFindingMethod theMethod) {
		...
		//find the  path between two selected nodes and select its edges and nodes
		//this method will only run if there are just two nodes selected in the graph
	...
	public ArrayList<Edge> findPathBetween(Node source, Node target, PathFindingMethod theMethod) {
		...
		if(theMethod == PathFindingMethod.Dijkstra) thePath = findPathUsingDijstra(source, target);
		else if(theMethod == PathFindingMethod.AStarLowerBound) thePath = findPathUsingAStarLowerBound(source, target);
		else if(theMethod == PathFindingMethod.AStarManhattan) thePath = findPathUsingAStarManhattan(source, target);
		else if(theMethod == PathFindingMethod.AStarZeroCostHeuristic) thePath = findPathUsingAStarZeroCostHeuristic(source, target);
		else if(theMethod == PathFindingMethod.AStarBestFirstGreedy) thePath = findPathUsingAStarBestFirstGreedy(source, target);
		else if(theMethod == PathFindingMethod.GreedyBestNeighbour) thePath = findPathUsingGreedyHeuristic(source, target);

		//indicate the path by selecting the edges in the graph that form the path
	...
	public ArrayList<Edge> findPathUsingGreedyHeuristic(Node source, Node target) {
		//This 
		// For illustration this method currently finds a path using an "As the crow flies" greedy
		// Heuristic. 
		// It simply chooses the next unvisited neighbour which is "as the crow flies" closest to the target
		// Because of this simple greedy behaviour without backtracking, it can dead end and get stuck 
		// with nowhere to go
		...
		while(currentNode != target){
			...
			for(Node n : neighbours){
				//find neighbour which is unvisited and "as the crow flies" closest to target
				n.setMarked(true); //mark node as having been examined by algorithm
	...
	//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&//
	//////////////////////////////////////////////////////////////////////////////////////////////
	//METHODS TO WRITE FOR ASSIGNMENT #5
		...
		while(currentNode != target){
			...
			for(Node n : currentNode.neighbours()){
				if(n.getVisited()==false){
					...
					Edge connectingEdge =edgeBetween(currentNode,n);//find corresponding edge
					int edgeLen =connectingEdge.getWeight();							
					//set distance if it is greater then where we have come reset
	...
	public Node smallestDistanceAStar(ArrayList<Node> a,Node target){
		...
		for(Node n:a){
			if(smallestNode==null||n.getDistance()+ distanceBetween(n,target)<smallestNode.getDistance()+ distanceBetween(smallestNode,target)){
				smallestNode=n;
			}
		}
	...
	public Node smallestDistanceManhattan(ArrayList<Node> a,Node target){
		...
		for(Node n:a){
			if(smallestNode==null||n.getDistance()+ Math.abs(n.getLocation().x - target.getLocation().x)+
													Math.abs(n.getLocation().y - target.getLocation().y)
								   <smallestNode.getDistance()+smallX+smallY){
